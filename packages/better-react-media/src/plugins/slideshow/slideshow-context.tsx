...
import type { ComponentProps, SlideshowRef, SlideStatus } from '../../index';
import {
...
export const useSlideshow = makeUseContext(
  'useSlideshow',
  'SlideshowContext',
  SlideshowContext
);
...
export function SlideshowContextProvider({
  ...
  const slideStatus = React.useRef<SlideStatus>();
  ...
  const disabled =
    slides.length === 0 || (finite && currentIndex === slides.length - 1);
  ...
  React.useEffect(scheduleNextSlide, [
    currentIndex,
    playing,
    scheduleNextSlide,
  ]);
