import * as React from 'react';

import type { ImageSlideProps, ImageSource, SlideImage } from '../../index';
import {
  devicePixelRatio,
  ImageSlide,
  isImageFitCover,
  mergeCompositorImageProps,
  UNKNOWN_ACTION_TYPE,
  useEventCallback,
  useLayoutEffect,
} from '../../index';

export type ResponsiveImageSlide = Omit<SlideImage, 'srcSet'> & {
  srcSet: [ImageSource, ...ImageSource[]];
};

export function isResponsiveImageSlide(
  slide: SlideImage
): slide is ResponsiveImageSlide {
  return (slide.srcSet?.length || 0) > 0;
}

export type ResponsiveImageProps = Omit<ImageSlideProps, 'slide' | 'rect'> &
  Required<Pick<ImageSlideProps, 'rect'>> & {
    slide: ResponsiveImageSlide;
    interactive: boolean;
  };

interface ResponsiveImageState {
  current?: string;
  preload?: string;
}

interface ResponsiveImageStateAction {
  type: 'fetch' | 'done';
  source: string;
}

function reducer(
  { current, preload }: ResponsiveImageState,
  { type, source }: ResponsiveImageStateAction
): ResponsiveImageState {
  switch (type) {
    case 'fetch': {
      if (!current) {
        return { current: source };
      }
      return { current, preload: source };
    }
    case 'done': {
      if (source === preload) {
        return { current: source };
      }
      return { current, preload };
    }
    default: {
      throw new Error(UNKNOWN_ACTION_TYPE);
    }
  }
}

export function ResponsiveImage(props: ResponsiveImageProps) {
  const [{ current, preload }, dispatch] = React.useReducer(reducer, {});

  const {
    slide: image,
    rect,
    imageFit,
    render,
    interactive,
    imageProps,
  } = props;

  const compositorImageProps = React.useMemo(
    () => mergeCompositorImageProps(imageProps, !interactive),
    [imageProps, interactive]
  );

  const srcSet = image.srcSet.toSorted((a, b) => a.width - b.width);
  const width = image.width ?? srcSet.at(-1).width;
  const height = image.height ?? srcSet.at(-1).height;
  const cover = isImageFitCover(image, imageFit);
  const maxWidth = Math.max(...srcSet.map((x) => x.width));
  const targetWidth = Math.min(
    (cover ? Math.max : Math.min)(rect.width, width * (rect.height / height)),
    maxWidth
  );
  const pixelDensity = devicePixelRatio();

  const handleResize = useEventCallback(() => {
    const targetSource =
      srcSet.find((x) => x.width >= targetWidth * pixelDensity) ??
      srcSet.at(-1);

    if (
      !current ||
      srcSet.indexOf(targetSource) < srcSet.findIndex((x) => x.src === current)
    ) {
      dispatch({ source: targetSource.src, type: 'fetch' });
    }
  });

  useLayoutEffect(handleResize, [
    rect.width,
    rect.height,
    pixelDensity,
    handleResize,
  ]);

  const handlePreload = useEventCallback((currentPreload: string) =>
    dispatch({ source: currentPreload, type: 'done' })
  );

  const style = {
    // workaround occasional flickering in mobile Safari
    WebkitTransform: interactive ? 'initial' : 'translateZ(0)',
  };

  if (!cover) {
    Object.assign(
      style,
      rect.width / rect.height < width / height
        ? { height: 'auto', width: '100%' }
        : { height: '100%', width: 'auto' }
    );
  }

  return (
    <>
      {preload && preload !== current && (
        <ImageSlide
          key="preload"
          {...props}
          imageProps={compositorImageProps}
          // do not publish active slide status events
          offset={undefined}
          slide={{ ...image, src: preload, srcSet: undefined }}
          style={{ position: 'absolute', visibility: 'hidden', ...style }}
          onLoad={() => handlePreload(preload)}
          render={{
            ...render,
            iconError: () => null,
            iconLoading: () => null,
          }}
        />
      )}

      {current && (
        <ImageSlide
          key="current"
          {...props}
          imageProps={compositorImageProps}
          slide={{ ...image, src: current, srcSet: undefined }}
          style={style}
        />
      )}
    </>
  );
}
