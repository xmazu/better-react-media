...
export const useA11yContext = makeUseContext(
  'useA11yContext',
  'A11yContext',
  A11yContext
);
...
export function A11yContextProvider({ children }: React.PropsWithChildren) {
  ...
  const context = React.useMemo(() => {
    const trackFocusWithin: A11yContextType['trackFocusWithin'] = (
      onFocus,
      onBlur
    ) => {
      const trackAndDelegate =
        (focusWithinValue: boolean) => (event: React.FocusEvent) => {
    ...
    return { autoPlaying, focusWithin, setAutoPlaying, trackFocusWithin };
