...
export type Subscribe = <T extends Topic>(
  topic: T,
  callback: EventCallback<T>
) => () => void;

export type Unsubscribe = <T extends Topic>(
  topic: T,
  callback: EventCallback<T>
) => void;
...
export const EventsContext = React.createContext<EventsContextType | null>(
  null
);

export const useEvents = makeUseContext(
  'useEvents',
  'EventsContext',
  EventsContext
);
...
export function EventsProvider({ children }: React.PropsWithChildren) {
  ...
  React.useEffect(
    () => () => {
      (Object.keys(subscriptions) as Topic[]).forEach(
  ...
  const context = React.useMemo<EventsContextType>(() => {
    const unsubscribe = <T extends Topic>(
      topic: T,
      callback: EventCallback<T>
    ) => {
    ...
    const subscribe = <T extends Topic>(
      topic: T,
      callback: EventCallback<T>
    ) => {
