...
import type { UseSensors } from '../../hooks/use-sensors';
...
export function usePointerSwipe<T extends Element = Element>(
  ...
  const lookupPointer = React.useCallback(
    (event: React.PointerEvent) =>
      pointers.current.find(({ pointerId }) => event.pointerId === pointerId),
    []
  );
    ...
    if (pointer) {
      if (activePointer.current === event.pointerId) {
        ...
        if (
          ...
          (hasDomClass(target, domClasses.slide) ||
            hasDomClass(target, domClasses.slideWrapper))
        ) {
          onClose();
        }
      }
