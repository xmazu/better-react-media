...
function currentTransformation(node: HTMLElement) {
  ...
  const matcher = matrix.match(/matrix.*\((?<values>.+)\)/u);
  if (matcher) {
    const values = matcher.groups.values.split(',').map(parseInt);
    if (values.length === 6) {
      x = values[4];
      y = values[5];
  ...
  useLayoutEffect(() => {
    if (nodeRef.current && snapshot.current !== undefined && !reduceMotion) {
      ...
      if (keyframes && duration) {
        ...
        if (typeof startTransform === 'string') {
          nodeRef.current.style.transform = startTransform;
        }

        try {
          animation.current = nodeRef.current.animate?.(keyframes, {
            duration,
            easing,
            fill: 'both',
          });
