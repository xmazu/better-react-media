...
export type SubscribeSensors<T extends Element> = <
...
export interface UseSensors<T extends Element> {
...
export function useSensors<T extends Element>(): UseSensors<T> {
  ...
  const notifySubscribers = React.useCallback(
    <ET extends SupportedEventType>(type: ET, event: ReactEventType<T, ET>) => {
      (
        subscribers[type] as SensorCallback<T, ReactEventType<T, ET>>[]
      )?.forEach((listener) => {
  ...
  const registerSensors = React.useMemo(
    () => ({
      onKeyDown: (event: React.KeyboardEvent<T>) =>
        notifySubscribers(EVENT_ON_KEY_DOWN, event),
      onKeyUp: (event: React.KeyboardEvent<T>) =>
        notifySubscribers(EVENT_ON_KEY_UP, event),
      ...
      onPointerUp: (event: React.PointerEvent<T>) =>
        notifySubscribers(EVENT_ON_POINTER_UP, event),
      onWheel: (event: React.WheelEvent<T>) =>
        notifySubscribers(EVENT_ON_WHEEL, event),
    }),
    [notifySubscribers]
  );
  ...
  const subscribeSensors = React.useCallback(
    <ET extends SupportedEventType>(
      ...
      (subscribers[type] as SensorCallback<T, ReactEventType<T, ET>>[]).unshift(
        callback
      );

      return () => {
        const listeners = subscribers[type] as SensorCallback<
          T,
          ReactEventType<T, ET>
        >[];
        if (listeners) {
