...
import { Fullscreen, Thumbnails } from '../../src/plugins/index';
import {
  clickButton,
  expectCurrentImageToBe,
  expectToContainButton,
  lightbox,
  queryThumbnails,
} from "../test-utils";
...
describe("Thumbnails", () => {
  ...
  it("supports custom thumbnails button", () => {
    renderLightbox({
      render: {
        buttonThumbnails: () =>
          React.createElement("button", { type: "button" }, "Custom button"),
      },
  ...
  it("supports custom thumbnails", () => {
    renderLightbox({
      render: {
        thumbnail: () => React.createElement("img", { src: "custom" }),
      },
    });
  ...
  it("renders correct number of thumbnails", () => {
    ...
    const generateTestCases = (finite: boolean, testCases: number[][]) =>
      testCases.flatMap((expectedThumbnails, preload) =>
        expectedThumbnails.map(
          (expected, slides) => [slides, preload, finite, expected] as const
        )
      );
    ...
    for (const [slides, preload, finite, expected] of [
      ...
      rerender(
        lightbox({
          ...
          plugins: [Thumbnails],
          slides: Array.from({ length: slides }).map((_, index) => ({
            src: `img${index}`,
          })),
        })
      );
      ...
      expect(
        queryThumbnails().length,
        JSON.stringify({ expected, finite, preload, slides })
      ).toBe(expected);
    }
  });
});
